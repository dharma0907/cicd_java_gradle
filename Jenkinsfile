            ...
            //here we are building docker image and pusing image 
                steps{
                    script{
                       ...
                       sh 'docker build -t  34.125.102.180:8083/springboot:${version} .'
                       sh 'docker login -u admin -p $dockerpass 34.125.102.180:8083'
                       sh 'docker push  34.125.102.180:8083/springboot:${version}'
                       sh 'docker rmi  34.125.102.180:8083/springboot:${version}' 
              ...
              steps {
                 script{
                         withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: '', contextName: '', credentialsId: 'kubeconfig', namespace: '', serverUrl: 'https://10.182.0.5:6443']])  {
                          dir('kubernetes/') {
                            sh 'helm upgrade --install --set image.repository="34.125.102.180:8083/springapp" --set image.tag="${VERSION}" myjavaapp myapp/ '
                        }
                    }  
                }
             }
